...
module USQualityCoreTestKit
  module Client
    module USQualityCoreClientV050
      class SpecimenNonRequiredSearchParametersClientSearchTest < Inferno::Test
        include TestHelper

        id :us_quality_core_v050_specimen_non_required_search_parameters_client_search_test

        title 'SHOULD only use required search parameters for Specimen'
        ...
        run do
          ...
          skip_if requests.blank?, "Inferno did not receive any search requests for the `Specimen` resource type."

          non_required_search_parameters(requests, required_search_parameters).each do |parameter|
            warning "The client used the non-required search parameter `#{parameter}` for `Specimen`. " \
