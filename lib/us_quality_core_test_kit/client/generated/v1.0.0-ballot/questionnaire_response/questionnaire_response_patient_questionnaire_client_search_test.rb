  ...
  module Client
    module USQualityCoreClientV100_BALLOT
      class QuestionnaireResponsePatientQuestionnaireClientSearchTest < Inferno::Test
        ...
        id :us_quality_core_v100_ballot_questionnaire_response_patient_questionnaire_client_search_test

        title 'SHALL support patient + questionnaire search of QuestionnaireResponse'

        description %(
          The client demonstrates SHALL support for searching patient + questionnaire on QuestionnaireResponse.
        )
        ...
        run do
          ...
          non_required_search_parameters(requests_with_params, all_required_search_parameters).each do |parameter|
            warning "The client used the non-required search parameter `#{parameter}` for `QuestionnaireResponse`. " \
