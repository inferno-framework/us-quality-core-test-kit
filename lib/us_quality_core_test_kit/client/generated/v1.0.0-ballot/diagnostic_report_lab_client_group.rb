# frozen_string_literal: true

require_relative 'diagnostic_report_lab/diagnostic_report_lab_client_read_test'
require_relative 'diagnostic_report_lab/diagnostic_report_lab_patient_category_client_search_test'
require_relative 'diagnostic_report_lab/diagnostic_report_lab_patient_client_search_test'
require_relative 'diagnostic_report_lab/diagnostic_report_lab_patient_category_date_client_search_test'
require_relative 'diagnostic_report_lab/diagnostic_report_lab_patient_code_client_search_test'

module USQualityCoreTestKit
  module Client
    module USQualityCoreClientV100_BALLOT
      class DiagnosticReportLabClientGroup < Inferno::TestGroup
        id :us_quality_core_client_v100_ballot_diagnostic_report_lab

        title 'DiagnosticReport for Laboratory Results Reporting'

        description %(

# Background

This test group verifies that the client can access DiagnosticReport data
conforming to the US Quality Core DiagnosticReport Profile for Laboratory Results Reporting Profile.

# Testing Methodology

## Data Access Supported

Clients may not be required to support the DiagnosticReport FHIR resource type. However, if they
do support it, they must support the US Quality Core DiagnosticReport Profile for Laboratory Results Reporting Profile and the resource type's search parameters.
The tests in this group will not execute if client makes no attempt to access data for the
DiagnosticReport resource type. In this case, the test will be marked as skip if support
for the resource type is required, and omitted otherwise.

## Reading
This test will check that the client performed a read of the following id:

* `usqualitycore-diagnostic-report-lab`

## Searching
These tests will check that the client performed searches against the
DiagnosticReport resource type with the following required parameters:

* patient + category
* patient
* patient + category + date
* patient + code

Inferno will also look for searches using the following optional parameters:




        )

        optional false

        run_as_group

        test from: :us_quality_core_v100_ballot_diagnostic_report_lab_client_read_test
        test from: :us_quality_core_v100_ballot_diagnostic_report_lab_patient_category_client_search_test
        test from: :us_quality_core_v100_ballot_diagnostic_report_lab_patient_client_search_test
        test from: :us_quality_core_v100_ballot_diagnostic_report_lab_patient_category_date_client_search_test
        test from: :us_quality_core_v100_ballot_diagnostic_report_lab_patient_code_client_search_test
      end
    end
  end
end
