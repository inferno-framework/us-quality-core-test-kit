...
module USQualityCoreTestKit
  module Client
    module USQualityCoreClientV100_BALLOT
      class ImagingStudySubjectClientSearchTest < Inferno::Test
        include TestHelper

        id :us_quality_core_v100_ballot_imaging_study_subject_client_search_test

        title 'SHALL support subject search of ImagingStudy'

        description %(
          The client demonstrates SHALL support for searching subject on ImagingStudy.
        )
        ...
        run do
          ...
          non_required_search_parameters(requests_with_params, all_required_search_parameters).each do |parameter|
            warning "The client used the non-required search parameter `#{parameter}` for `ImagingStudy`. " \
