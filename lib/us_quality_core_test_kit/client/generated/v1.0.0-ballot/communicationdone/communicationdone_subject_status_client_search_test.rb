...
module USQualityCoreTestKit
  module Client
    module USQualityCoreClientV100_BALLOT
      class CommunicationdoneSubjectStatusClientSearchTest < Inferno::Test
        include TestHelper

        id :us_quality_core_v100_ballot_communicationdone_subject_status_client_search_test

        title 'SHALL support subject + status search of Communicationdone'

        description %(
          The client demonstrates SHALL support for searching subject + status on Communicationdone.
        )
        ...
        run do
          ...
          non_required_search_parameters(requests_with_params, all_required_search_parameters).each do |parameter|
            warning "The client used the non-required search parameter `#{parameter}` for `Communication`. " \
