      ...
      if @missing_elements.any?
        ...
        support_practitioner_role = practitioners.any? do |practitioner|
          ...
          target_profile_with_version =
            "http://hl7.org/fhir/us/quality-core/StructureDefinition/us-quality-core-practitionerrole|#{metadata.profile_version}"

          practitioner_roles.any? { |pr| resource_is_valid_with_target_profile?(pr, target_profile_with_version) }
        end
      end

      missing_must_support_message = 'Could not find US Quality Core PractitionerRole Profile resources and ' \
                                     "these MustSupport elements #{missing_elements_string.join(', ')} " \
                                     'in US Core Quality Practitioner Profile resources. ' \
