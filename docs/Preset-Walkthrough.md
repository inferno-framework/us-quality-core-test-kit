...
4. Enter a stable **Client Id**, such as `us-quality-core-demo-client`.
5. Inferno will display a FHIR base URL under `/custom/us_quality_core_client_v050/fhir`.
   Configure the client under test to use that URL.
6. Configure the client to send:

   ```text
   Authorization: Bearer us-quality-core-demo-client
   ```

   The bearer token must match the **Client Id** entered in Inferno so Inferno
   can associate requests with the current test run.
...
* `client_id`: the value Postman sends as `Authorization: Bearer <client_id>`.
  This must match the **Client Id** entered in Inferno for the current client
  test run.
...
4. Enter a **Client Id**, such as `postman-demo`.
5. Set the collection `client_id` variable to the same value.
6. Confirm that `base_url` matches the URL shown by Inferno.
7. Run the collection in Postman.
8. Return to Inferno and click the completion link in the **User Action
   Required** dialog.
9. Run the generated profile groups to evaluate the recorded Postman requests.
